  ...
  $(document).ready(
    ...
    await firebase.auth().onAuthStateChanged((user) => {
      if (user) {
          // ログイン状態
        uid = user.uid;
        div2 = $(`<a class="post_btn"><i id="post_icon" class="fa-solid fa-pen fa-lg"></i>投稿</a>`);
        db.collection("user").doc(uid).get().then((doc)=>{
          if(doc.get('profile') != '') {
            img = doc.get('profile');
          } else {
            img = "{{url_for('static', filename='image/noImage.png')}}";
          }
          ...
          // ログインしていない状態
          div2 = $('<a href="login.html" class="login">ログイン</a><a href="sign_up.html" class="sign_up">新期登録</a>');
      }
      $("#sign_in_btn").append(div2); 
      getWorkPost();
    });
        ...
        query.forEach((doc) => {
          ...
          if(doc.data().images.length > 0) {
           post_img = doc.data().images[0];
            ...
            post_img = "{{url_for('static', filename='image/noImage_post.png')}}";
          }

          var div = $(`<div class="post" onclick="workDetail('${docId}')">
                        <div class="content">
                          <h6>${industry}</h6>
                          <h6>${area}</h6>
